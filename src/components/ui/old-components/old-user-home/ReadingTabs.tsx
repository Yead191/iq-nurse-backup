  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex bg-gray-100  rounded-lg lg:w-full md:max-w-xl px-3 py-[6px]">
          {tabData.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-2 px-1 md:px-3 py-2 rounded-md text-sm font-medium transition-all flex-1 justify-center ${
                activeTab === tab.id
