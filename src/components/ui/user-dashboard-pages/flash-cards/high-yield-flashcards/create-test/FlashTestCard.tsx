...
const FlashTestCard: React.FC<Props> = ({ isFlipped, onFlip, card }) => (
  <div
    className={`relative w-full lg:h-[55vh] h-[45vh]  cursor-pointer transition-transform duration-700 transform-style-preserve-3d ${
    ...
    <div className="absolute inset-0 backface-hidden rounded-lg shadow rotate-y-180">
      <div className="w-full h-full bg-white rounded-lg p-6 md:p-8 flex flex-col">
        <div className="flex-1 flex flex-col items-center justify-center gap-y-4">
          <div className="lg:w-[45%] w-full text-center text-sm">{card.backContent.text}</div>
          <div className="w-auto lg:h-[248px] h-[192px] ">
            <img
