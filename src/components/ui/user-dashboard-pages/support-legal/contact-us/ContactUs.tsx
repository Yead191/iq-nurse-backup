...
const ContactUs = () => {
    return (
        <div>
            ...
            <SupportBanner title='Get In Touch' subTitle='Contact Us'
            ...
            <div className=' border border-[#C5D0D0] mt-6 lg:p-12 p-5 rounded-xl  '> 
                <p className='font-medium lg:text-xl text-lg lg:pb-8 pb-4'> Let's Connect </p> 

            <div className='grid lg:grid-cols-2 grid-cols-1 lg:gap-8 gap-4'>
                {
                    supportData?.map((values, index) => (
                        <div key={index} className="flex items-center p-4 bg-white rounded-xl  border border-[#C5D0D0]">
                            {/* Icon */}
                            <div className="flex items-center justify-center w-12 h-12 rounded-lg" style={{ backgroundColor: values?.bgColor }}>
                                <img src={values?.icon} alt={`${values?.label} Icon`} className="w-6 h-6" />
                            </div>
                            ...
                            <div className="ml-4">
                                ...
                                <a
                                    href={values?.label === "Email Support" ? `mailto:${values?.value}` : "#"}
