...
const AllFaqs = () => {
  ...
  const currentFaqs =
    questionsAndAnswers[activeTab]?.slice(indexOfFirstItem, indexOfLastItem) ||
    [];
  ...
  const getItems = (panelStyle: CSSProperties) =>
    currentFaqs.map((faq, index) => ({
      key: String(index + 1),
      label: (
        <p className="text-[16px] font-normal text-[#4E4E4E]">{faq.question}</p>
      ),
    ...
    <div className="lg:py-8 py-4">
      ...
      <div className="flex space-x-4 lg:mb-6 lg:justify-center justify-start  overflow-x-auto lg:pb-3">
        {Object.keys(questionsAndAnswers).map((tab) => (
          <button
            key={tab}
            className={` lg:text-[16px] text-sm font-semibold h-[46px] lg:w-[168px] rounded-lg px-4  ${
              activeTab === tab
                ? " text-white bg-[#2C5F8D]  "
      ...
      <div className=" lg:hidden block py-8 ">
        <SupportBanner
          ...
          description="Every thing you need to know about Ace-Nursing"
        />
      </div>
