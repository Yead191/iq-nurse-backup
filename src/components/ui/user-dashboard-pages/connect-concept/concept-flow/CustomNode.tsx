...
const CustomNode = ({ data }: CustomNodeProps) => {
    ...
    return (
        ...
        >
            <div className="w-[270px] min-h-[100px] rounded-md  border-s-4 bg-white shadow border border-gray-100 p-2 font-sans relative"
                style={{ borderLeftColor: color }}>
                <div className="flex items-center gap-2 pb-2">
                    <button type="button" className="h-5 w-5 rounded" style={{ backgroundColor: `${color}80` }} />
                    <p className="text-base font-bold text-black">{data.label}</p>
                </div>
                {
                    data.description ? (
                        <span className="text-sm text-gray-500 italic"> - {data.description}</span>
                    ) : null
                }
                ...
                <div className={editing ? "nodrag" : "hidden"}>
                    ...
                    <button
                        ...
                        style={{ backgroundColor: color, color: "white", padding: "0.25rem 0.75rem", borderRadius: "0.375rem" }}
                    >
                        <FiSave /> Save
                    </button>
                </div>
