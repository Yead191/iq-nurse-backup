...
import {
  sheetSidebarItems,
  NavigationItem,
} from "@/data/cheat-sheets/sheetSidebarItems";
...
export default function SheetsSidebar() {
  ...
  const setActiveSection = (topicId: string) => {
    router.push(`/profile/cheat-sheets/${topicId}`);
  };
          ...
          <div className="flex-1 text-left">
            ...
            {item.count !== undefined && (
              <p className="text-[9px] font-normal text-[#7f8c8d] leading-[14px]">
                {item.count} {item.count === 1 ? "Sheet" : "Sheets"}
              </p>
            )}
          </div>
  ...
  const filteredData = filterItems(sheetSidebarItems);
  ...
  return (
    <div className="h-full flex flex-col bg-white w-full lg:w-64 2xl:w-80  lg:border-r lg:border-gray-200  lg:h-[calc(100vh-64px)] overflow-auto">
      ...
      <div className="hidden lg:flex lg:px-4 pt-4 pb-2 border-b border-gray-200 space-y-2">
        <button
