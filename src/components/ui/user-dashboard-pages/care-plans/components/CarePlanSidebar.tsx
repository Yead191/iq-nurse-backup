...
export default function CarePlanSidebar() {
  ...
  useEffect(() => {
    ...
    const isBasePath = pathname === "/profile/care-plans" || last === "care-plans"
      ...
      {carePlansCategories?.map((cat) => (
        <div key={cat.id} className="mb-2">
          ...
          <div
            ...
            ${selectedCategory === cat.id ? "bg-[#E8EBFB] border border-[#02478D]" : "hover:bg-gray-50"}`}
          >
            <div className="flex items-center gap-3">
              <Folder style={{ color: cat.color }} className={`w-8 h-8 fill-current`} />
              <div>
                <div className="font-medium text-gray-900 text-sm">{cat.name}</div>
                <div className="text-xs text-gray-500">{cat.topicCount} topics</div>
              </div>
            </div>
