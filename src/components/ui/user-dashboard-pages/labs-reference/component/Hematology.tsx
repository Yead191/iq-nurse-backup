...
import { hematologyData } from "@/data/labsReferenceData";
import { Copy, Activity, AlertCircle, TrendingUp, TrendingDown, Info, AlertTriangle, BookOpen } from "lucide-react";
            ...
            <div className="space-y-12">
                {hematologyData.map((item, index) => (
                    <div className="bg-[#f8f9fa] border-l-4 border-[#2c5f8d] p-5 rounded-lg shadow-sm">
                        <div key={item.id} className="border-b last:border-b-0 pb-10 last:pb-0">
                            {/* Section Header with Badge */}
                            <div className="flex flex-wrap items-center gap-2 mb-4">
                                <h3 className="text-lg font-bold text-[#1565C0]">{item.name}</h3>
                                {item.tags?.map((tag, i) => (
                                    <span key={i} className={`px-2 py-0.5 rounded text-xs font-bold text-white ${tag === "NCLEX" ? "bg-[#26C6DA]" : "bg-[#FFA000]"}`}>
                                        {tag}
                                    </span>
                                ))}
                            </div>
                            ...
                            <div className="space-y-4 mb-6">
                                <div className="bg-white  rounded-lg p-4 shadow-sm">
                                    <h4 className="flex items-center gap-2 font-bold text-[#1E88E5] mb-2">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                {/* Elevated */}
                                <div className="bg-white  rounded-lg p-4 shadow-sm">
                                    <h4 className="flex items-center gap-2 font-bold text-[#1976D2] mb-3">
                            ...
                            <div className="bg-white  rounded-lg p-4 shadow-sm mb-6">
                                <h4 className="flex items-center gap-2 font-bold text-[#1565C0] mb-3">
                            ...
                            {item.medications && (
                                <div className="bg-white  rounded-lg p-4 shadow-sm mb-6">
                                    ...
                                    <div className="space-y-2">
                                        <div>
                                            <span className="font-bold text-gray-800">• Increase:</span>
                                            <p className="text-gray-700 text-sm ml-3 text-wrap">{item.medications.increase}</p>
                                        </div>
                                        <div>
                                            <span className="font-bold text-gray-800">• Decrease:</span>
                            ...
                            {item.nclexTip && (
                                <div className="bg-[#E0F7FA] border border-[#80DEEA] rounded-lg p-4">
                                    <h4 className="flex items-center gap-2 font-bold text-[#00838F] mb-2 uppercase text-xs">
                                        <div className="text-[#00838F] font-bold">⚡</div> {item.nclexTip.title}
                                    </h4>
