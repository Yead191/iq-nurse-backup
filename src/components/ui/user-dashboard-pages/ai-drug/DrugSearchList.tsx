...
export default function DrugSearchList({
  ...
  return (
    <aside className="w-full sm:w-sm scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100  m-auto">
      ...
      <div
        className="bg-white p-3 h-[calc(100vh-220px)] overflow-y-auto "
        style={{ boxShadow: "4px 4px 35px 0px #00000021" }}
      >
        {/* Drug Card Search History */}
        <div className="space-y-2 ">
          <Tooltip
            title={showHistory ? "Hide Search History" : "Show Search History"}
          >
            <div className="flex items-center gap-2 mb-2 mt-2 group">
              <button
                ...
                aria-label={
                  showHistory
                    ? "Hide Medication Search History"
                    : "Show Medication Search History"
                }
          ...
          {filteredMedicationList.length > 0 ? (
            filteredMedicationList.map((drug, idx) => (
              <div
                key={drug.id + idx}
                ...
                onClick={() => {
                  ...
                  // setIsSideBarSelect(true)
                  router.push(`/profile/ai-drug/details/${drug.id}`);
                }}
              >
                <span className="text-[#F59E42] text-lg mt-0.5">💊</span>
                <div>
